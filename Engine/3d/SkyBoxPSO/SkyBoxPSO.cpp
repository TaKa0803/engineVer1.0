...
#pragma region ShaderをCompileする
	...
	//Shaderをコンパイルする
	IDxcBlob* vertexShaderBlob = CompileShader(L"resources/shaders/SkyBox.VS.hlsl", L"vs_6_0", DXC->GetDxcUtils(), DXC->GetDxcCompiler(), DXC->GetIncludeHandler());
	assert(vertexShaderBlob != nullptr);

	IDxcBlob* pixelShaderBlob = CompileShader(L"resources/shaders/SkyBox.PS.hlsl", L"ps_6_0", DXC->GetDxcUtils(), DXC->GetDxcCompiler(), DXC->GetIncludeHandler());
...
#pragma region DepthStencilStateの設定を行う
	...
	//書き込みします
	depthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;//全ピクセルがz=1に出力されるのでわざわざ書きもむ必要なし
...
void SkyBoxPSO::PreDraw(FillMode fillmode, BlendMode blendMode)
{
	...
	DXF_->GetCMDList()->SetPipelineState(graphicsPipelineState_);
}
